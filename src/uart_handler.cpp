...
#define UART_RX_PIN 5   // IO5 - RX2
#define UART_TX_PIN 17  // IO17 - TX2
...
String lastResponse = "";
UARTStatistics uartStats = {0, 0, 0, 0, 0, 100.0};  // İstatistikler başlangıç değerleri
...
bool sendBaudRateCommand(long baudRate) {
    ...
    uartStats.totalFramesSent++;  // İstatistik güncelle
...
String safeReadUARTResponse(unsigned long timeout) {
        ...
        if (UART_PORT.available()) {
            ...
            if (c == '\n' || c == '\r') {
                if (response.length() > 0) {
                    uartStats.totalFramesReceived++;  // İstatistik güncelle
                    return response;
                }
    ...
    if (response.length() == 0) {
        uartStats.timeoutErrors++;  // Timeout hatası
    }
